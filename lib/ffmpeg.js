...
async function main(opts) {
  ...
  if (!await confirmOutput(ffmpeg, opts)) return opts.output;
  ...
  const processDurationString = _.prettyMs(processDuration);
...
async function ensureOutput(ffmpeg, opts) {
  ...
  if (!output) {
    ...
    output = opts.output || _.pathFrom(opts.input, { extname: e => '_converted' + e });
...
async function confirmOutput(ffmpeg, opts) {
  ...
  if (error) {
    ...
    console.error(error);
    if (opts.yes || await confirm(`Remove unreadable output "${opts.output}"`)) {
  ...
  const diffString = _.prettyS(diff);
...
async function probeOutput(opts) {
  ...
  function log({
    ...
    logger([
      `${output}"${opts.output}"`,
      diff.isSizeSignificant && `${size}${_.prettyBytes(probe.size)} (${diff.sizeString})`,
      diff.isDurationSignificant && `${duration}${_.prettyS(probe.duration)} (${diff.durationString})`,
    ].filter(Boolean).join('\n'));
  }
...
function onProgress(opts) {
  ...
  return progress => {
    ...
    if (progress.timemark) {
      ...
      progress.percent = Math.floor((duration / supposedOutputDuration) * 100);
      pretty.percent = `${progress.percent}%`;
      pretty.timemark = progress.timemark;
    }
    if (progress.currentKbps) {
      progress.bitrate = parseInt(progress.currentKbps);
      pretty.bitrate = `Bitrate: ${progress.currentKbps} kbps`;
    }
    if (progress.targetSize) {
      progress.size = parseInt(progress.targetSize) * 1024;
      pretty.size = `Size: ${_.prettyBytes(progress.targetSize* 1024)}`;
    }
    if (progress.currentFps) {
      progress.fps = Number(progress.currentFps);
      pretty.fps = `FPS: ${progress.currentFps}`;
    }
    // string.push(`${progress.currentKbps} kbps`);
    if (progress.percent) {
      const ratio = progress.ratio = progress.percent / 100;
      Object.assign(progress, eta({ ratio }));
      pretty.elapsed = `Elapsed: ${_.prettyMs(progress.elapsed)}`;
      pretty.remaining = `ETA: ${_.prettyMs(progress.remaining)}`;
    }
