...
let userConfig;
let userPresets;
let userPresetAliases; // alias => actual preset name
try {
  ...
  if (userPresets) {
    ...
    for (const [name, preset] of Object.entries(userPresets)) {
      ...
      let aliases = preset.alias || preset.aliases; // support either key
      if (!aliases) continue;
      if (!Array.isArray(aliases)) aliases = [aliases];
      for (const alias of aliases) {
        if (typeof alias !== 'string' || !alias.trim()) continue;
        if (userPresets[alias]) {
          console.warn(`Preset alias '${alias}' conflicts with an existing preset name; ignoring.`);
          continue;
        }
        if (userPresetAliases[alias] && userPresetAliases[alias] !== name) {
          console.warn(`Preset alias '${alias}' already mapped to '${userPresetAliases[alias]}', cannot also map to '${name}'; ignoring.`);
          continue;
        }
        userPresetAliases[alias] = name;
      }
    }
  }
} catch (error) {
...
async function initializeOpts(opts) {
  ...
  if (opts.preset) {
    ...
    if (!userPresets?.[presetName] && userPresetAliases?.[presetName]) {
      presetName = userPresetAliases[presetName];
      if (!opts.quiet) console.log(`Using preset '${presetName}' (alias: '${opts.preset}')`);
    }
    if (userPresets?.[presetName]) {
      delete opts.preset;
      Object.assign(opts, userPresets[presetName]);
    }
  }
...
async function applyOptions(ffmpeg, opts) {
  console.debug('[DEBUG] `applyOptions(`', {
    ffmpeg,
    opts,
    'ffmpeg.options.outputOptions': ffmpeg.options.outputOptions,
  });
  // process.exit()
  /* Apply Options */
  let videoFilters = [];
  ...
  /* Treble audio filter: maps to: -af "treble=g=5" (gain defaults to 5 unless a numeric value is provided) */
  if (opts.treble) {
    let gain = 5;
    if (typeof opts.treble === 'number' && !isNaN(opts.treble)) gain = opts.treble;
    audioFilters.push(`treble=g=${gain}`);
  }
  /* Voice enhancement preset: boosts presence (3kHz) and adds a highshelf around 8kHz */
  if (opts.voice) {
    // Allow custom object later; for now boolean enables default chain
    if (typeof opts.voice === 'object') {
      const v = opts.voice;
      const f1 = v.f1 || 3000;
      const w1 = v.width1 || 2000;
      const g1 = v.g1 || 4;
      const f2 = v.f2 || 8000;
      const g2 = v.g2 || 3;
      audioFilters.push(`equalizer=f=${f1}:t=h:width=${w1}:g=${g1},highshelf=f=${f2}:g=${g2}`);
    } else {
      audioFilters.push('equalizer=f=3000:t=h:width=2000:g=4,highshelf=f=8000:g=3');
    }
  }
  if (opts.framerate) {
  ...
  // Join audio filters with commas so they form a single filterchain.
  // Using semicolons created multiple independent chains without mapping, causing errors when adding `voice`.
...
function verticalAdjust(opts, filters) {
  if (!opts.verticalAdjust) return filters;
