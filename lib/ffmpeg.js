...
async function main(opts) {
  ...
  if (typeof opts.ffmpeg === 'function') {
    ...
    if (configuredOutput && configuredOutput !== opts.output) {
      console.warn(`Warning: Specified output ('${opts.output}') different from configured output ('${configuredOutput}')`);
    }
  }

  const checkOutputResult = await checkOutput(opts);

  if (checkOutputResult.cancelled) throw new Error(`User Cancelled: '${opts.input}'`);
  ...
  if (opts.output !== ffmpeg?._currentOutput?.target) {
    console.warn(`Warning: opts.output (='${opts.output}') !== ffmpeg.output(='${ffmpeg?._currentOutput?.target}')`);
  }
  try {
...
async function initializeOpts(opts) {
  ...
  if (!opts.output) {
    const input = opts.inputs?. [0] ?? opts.input
    let extension = opts.extension;
    if (!extension.startsWith('.')) extension = '.' + extension;
    if (typeof input === 'string') {
      opts.output = _.pathFrom(input, {
        extname: e => (opts.outputSuffix ?? '_converted')
          + (extension ?? e),
      });
    } else {
...
async function applyOptions(ffmpeg, opts) {
  ...
  if (videoFilters.length)
    ffmpeg.outputOptions('-vf', verticalAdjust(opts, _.dedupe(videoFilters)).join(','));
  else if (filterComplex.length)
    ffmpeg.outputOptions('-filter_complex', verticalAdjust(opts, _.dedupe(filterComplex)).join(','));
  ...
  if (opts?.inputMetadata?.comment) {
    ffmpeg.outputOptions('-metadata', `comment=${opts?.inputMetadata?.comment}`);
  }
  if ('rotateMeta' in opts) {
    ffmpeg.outputOptions('-metadata:s:v', `rotate=${opts.rotateMeta}`);
  }
...
async function checkOutput(opts) {
  ...
  try {
    ...
    if (!opts.output) {
      result.output = opts.output || _.pathFrom(opts.input, {
    ...
    try { result.probe = await ffprobe({ input: result.output }) } catch (x) { result.probe = false }
  ...
  async function rename() {
    ...
    while (true) {
      i++;
      const newName = _.pathFrom(result.output, { extname: e => ` (${i})` + e });
...
async function probeOutput(opts) {
  ...
  diff.isSizeSignificant = diff.size < .95 || 1.05 < diff.size;
  ...
  function log({
    ...
    logger([
      `${output}"${opts.output}"`,
      diff.isSizeSignificant && `${size}${_.humanSize(probe.size)} (${diff.sizeString} of ${_.humanSize(opts.inputSize)})`,
      diff.isDurationSignificant && `${duration}${_.humanDuration(probe.duration * 1000)} (${diff.durationString} of ${_.humanDuration(opts.inputDuration*1000)})`,
    ].filter(Boolean).join('\n'));
  }
