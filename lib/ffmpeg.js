...
async function main(opts) {
  ...
  if (opts.inputs?.length) {
    ...
    for (let i = 0; i < inputs.length; i++) {
      ...
      // console.log(`Processing input ${i+1} of ${inputs.length}`);
      try {
        // await main({ ...opts, inputs: null, input });
        ...
        const result = await runner.run({
          onProgress: {
            onProgress(currentProgress) {
              ...
              const totalRatio = (i + 1 + currentProgress.ratio) / runners.length
              // const totalRatioRefined = (totalRatio + currentProgress.ratio) / 2;
              // const totalRatioRefined = (totalRatio + currentProgress.ratio) / 2;
              const totalProgress = eta({ ratio: totalRatio });
              _.stdoutLine([
                totalProgress.percent
                && `${totalProgress.percent}% (current: ${currentProgress.percent}%)`,
...
async function applyOptions(ffmpeg, opts) {
  ...
  if (opts.videoScale)
    videoFilters.push(`scale=${opts.videoScale}`);
