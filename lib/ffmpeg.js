...
async function confirmOutput(ffmpeg, opts) {
  ...
  const diff = Math.abs((outputProbe.duration * (opts.speed || 1)) - opts.inputDuration);
  const isDiffLowEnough = (diff / 1000) < (opts.outputDDOT || 5);
  if (!opts.silent) {
    outputProbe.log({ output: 'Output exists: ' });
    if (!isDiffLowEnough) {
      if (opts.speed) {
        console.log(`Duration difference (taking speed into account): ${_.durationString(diff)}`);
      } else {
        console.log(`Duration difference (taking speed into account): ${_.durationString(diff)}`);
      }
    }
...
async function probeOutput(opts) {
  ...
  function log({
    ...
    logger([
      `${output}"${opts.output}"`,
      `${size}${_.sizeString(probe.size)} (${diff.sizeString})`,
