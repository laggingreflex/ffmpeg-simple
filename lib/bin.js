...
yargs.options({
  ...
  from: { type: ['number', 'string'], alias: ['ss'], description: 'Start time offset' },
  to: { type: ['number', 'string'], alias: [], description: 'Output stop time' },
  // format: { type: 'string', alias: [], description: 'Output stop time' },
  codec: { type: 'string', alias: ['c'], description: 'Codec' },
  fps: { type: ['number', 'string'], alias: ['r'], description: 'Input/output framerate' },
  ...
  preset: { type: 'string', alias: ['p'], description: 'Preset name (supports aliases defined in ~/ffmpeg-simple.json)' },
  ...
  overwrite: { type: 'boolean', alias: ['y'], description: 'Overwrite existing output' },
  help: { type: 'boolean', alias: 'h', description: 'Show help', },
  version: { type: 'boolean', alias: 'v', description: 'Show version number', },
});

yargs.command({
  command: ['convert [input] [output]', '$0 [input] [output]'],
  default: true,
  desc: 'Convert file',
  handler: ffmpeg,
});
...
yargs.command({
  command: 'cut <input> [from] [to] [output]',
  desc: 'Cut file',
  handler: cut,
});
