...
_.arrify = x => Array.isArray(x) ? x : typeof x === typeof undefined ? [] : [x];
...
_.all = async (array, cb, opts = {}) => {
  const total = array.length;
  const eta = _.eta({ total });
  const log = lodash.throttle(_.stdoutLine, 1000, { trailing: false });
  // const concurrency = opts.concurrency ?? OS.cpus().length ?? 1;
  const concurrency = 1;
  const results = [];
  for (let i_ = 0; i_ < array.length; i_ += concurrency) {
    const batch = array.slice(i_, i_ + concurrency);
    await Promise.all(batch.map(async (item, j) => {
      const i = i_ + j;
      try {
        const result = await cb(item, i)
        results[i] = result;
      } catch (error) {
        if (opts.halt !== false) throw error;
        if (opts.errors) opts.errors.push({ i, item, error });
      } finally {
        const e = eta();
        // log(`${e.percent}% ${e.count}/${total} ETA: ${e.string}`);
        // console.debug('allETA', { e });
        log([
          ...(e.message ? [
            e.message.replace('%', [
              '%',
  ...
  return _.promiseMap(array, async (item, i) => {
    try {
      ...
      if (currentProgress.elapsed >= 5000) {
        (opts?.log || _.stdoutLine)?.([opts?.prefix, eta().message, opts?.suffix].filter(Boolean).join(' '));
      }
    }
  }, opts);
};
...
_.remove = (path, opts = {}) => {
  const remove = () => {
    // if (!await confirm(`Permanently remove ${Array.isArray(path) ? `${path.length} files` : `"${path}"`}`)) return;
    if (!opts.silent) console.warn(`Permanently removing`, Array.isArray(path) ? `${path.length} files` : `"${path}"`);
    return _.all(_.arrify(path), f => fs.remove(f));
  };
  const recycle = async () => {
    try {
      return await trash(path);
    } catch (error) {
      if (!opts.silent) console.error(`Couldn't recycle`, Array.isArray(path) ? `${path.length} files` : `"${path}"`, error.message.substr(0, 100));
      return remove();
    }
  }
  try {
