 ...
 */
async function concatDemux({ input: inputs, ...opts }) {
  if (opts.log !== false) {
...
async function cut({ ...opts }) {
  if (opts.nearest) {
    const nearestKeyframe = await findNearestKeyframe(
      opts.input,
      opts.from || opts.ss
    );
    if (nearestKeyframe.nearest) {
      console.log(
        `Adjusting cut time from ${opts.from} to nearest keyframe at ${nearestKeyframe.nearest}...`
      );
      opts.from = opts.ss = nearestKeyframe.nearest;
    } else {
      console.log('No keyframes found, proceeding with original time...');
    }
  }
...
async function findNearestKeyframe(input, ss) {
  console.log(`Finding nearest keyframe...`);
  const probe = await ffprobe({
    input,
    extractKeyframes: true,
  });
  const keyFrames = probe.full.frames.filter(f => f.key_frame);
  ...
  for (const keyFrame of keyFrames) {
    const time = Number(keyFrame.best_effort_timestamp_time);
    if (time <= ss) {
      nearestKeyframes.early = time;
    }
    if (time >= ss && !nearestKeyframes.late) {
      nearestKeyframes.late = time;
    }
  }

  if (nearestKeyframes.early && nearestKeyframes.late) {
    const diffEarly = Math.abs(ss - nearestKeyframes.early);
    const diffLate = Math.abs(nearestKeyframes.late - ss);
    if (diffEarly <= diffLate) {
      nearestKeyframes.nearest = nearestKeyframes.early;
    } else {
      nearestKeyframes.nearest = nearestKeyframes.late;
    }
  }

  console.log(`Nearest keyframes at`, nearestKeyframes);

  return nearestKeyframes;
}
