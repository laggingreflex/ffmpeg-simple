...
module.exports = { concatDemux, concat, sample, sample2, cut, rotateMeta, caption, batch };
...
async function batch(opts) {
  if (!opts.dir) throw new Error('Need a dir');
  const dir = _.normalize(opts.dir);
  const files = await fs.readdir(dir);
  const total = files.length;
  console.log(`Processing ${total} files…`);
  for (let i = 0; i < total; i++) {
    const input = Path.join(dir, files[i]);
    console.log(`[${i+1}/${total}] ${input}`);
    const output = _.pathFrom(input, { dirname: Path.join(dir, 'output') });
    await ffmpeg({
      input,
      output,
      ...opts,
      // onProgress({ percent, pretty }) {
      //   const totalPercent = ((((i + 1) * 100) + percent) / total).toFixed(0);
      //   _.stdoutLine(`${totalPercent}$ [${i+1}/${total}] ${pretty}`);
      // },
    });
  }
}
